...
class Transformer(nn.Module):
    ...
    def __init__(self,
        ...
        decoder_layer = DecoderLayer(d_model=d_model,
                                     ...
                                     dropout=dropout)
        # self.encoder = Encoder(layer=encoder_layer, num_layers=enc_n_layers) ############### vit large  224 14 timm.
...
if __name__ == "__main__":
    ...
    # captions = torch.randint(10, 0, 128, (10, 30), dtype=torch.long)#.unsqueeze(0)
    captions = torch.randint(18022, (10, 128), dtype=torch.long) # 10, 30
    print(src_img.size())
    print(captions.size())
    # m_test = Transformer(52, 512, 196, 512, 2048, 2, 8, 8, 8, 30, 0.1, 0)
    # m_test = Transformer(vocab_size=18022, d_model=1024, img_encode_size=196, enc_ff_dim=512, dec_ff_dim=1024, enc_n_layers=2, dec_n_layers=6, enc_n_heads=8, dec_n_heads=8, max_len=128, dropout=0.1, pad_id=0)
    m_test = Transformer(vocab_size=18022, d_model=1024, dec_ff_dim=512, dec_n_layers=6, dec_n_heads=8, max_len=128, dropout=0.1, pad_id=0)
    print(m_test)
